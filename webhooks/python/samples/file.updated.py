...
MOVE_MAP = {
    "invoice": os.environ.get("FINANCE_FOLDER_ID"),
    "contract": os.environ.get("LEGAL_FOLDER_ID"),
    "nda": os.environ.get("AGREEMENTS_FOLDER_ID")
}
...
def webhook():
    ...
    app.logger.info(f'File processed: {trigger} -> "{title}" (Room ID: {room_id})')
    ...
    try:
        ...
        if is_sensitive:
            requests.post(
                ...
                json={
                    ...
                    "message": f'A sensitive file "{title}" has been uploaded or updated.\n\nRoom ID: {room_id}\nTime: {timestamp}'
                },
                headers={"Content-Type": "application/json"}
            )
        ...
        for keyword, target_folder_id in MOVE_MAP.items():
            if keyword in lower_title:
                ...
                app.logger.info(f'File "{title}" moved to folder for {keyword.upper()}')
                break  # Move only once based on the first match

        return "", 200

    except Exception as err:
        app.logger.error(f"Error during file processing: {err}")
        return "", 500
